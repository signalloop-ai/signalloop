...
export default function AdminReportDetail() {
  ...
  const [report, setReport] = useState<AdminEvidenceReport | null>(null);
  ...
  const loadReport = useCallback(async () => {
    ...
    setError(null);
    try {
      setReport(await fetchAdminReport(attemptId, getToken));
    } catch (loadError) {
  ...
  useEffect(() => {
    if (!isLoaded) return;
    const t = window.setTimeout(() => void loadReport(), 0);
  ...
  return (
    <main className="employer-page">
      ...
      {loading ? <p className="empty-state">Loading report…</p> : null}
